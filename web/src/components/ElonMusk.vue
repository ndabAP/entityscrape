...
export default {
  async mounted () {
    let { data: chart } = await axios.get('/api/entities?entity=Elon%20Musk')
    ...
    Plotly.newPlot('elon-musk-chart', [this.chart], this.layout, this.options)

    let { data: count } = await axios.get('/api/news?entity=Elon%20Musk')
