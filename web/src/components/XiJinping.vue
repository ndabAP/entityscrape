...
export default {
  async mounted () {
    let { data: chart } = await axios.get('/api/entities?entity=Xi%20Jinping')
    ...
    Plotly.newPlot('xi-jinping-chart', [this.chart], this.layout, this.options)

    let { data: count } = await axios.get('/api/news?entity=Xi%20Jinping')
